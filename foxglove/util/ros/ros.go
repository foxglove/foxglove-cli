...
// instead assumed to be 1. A ChinkInfo record is placed in the index section of
...
func parseMessage(header, data []byte) (*Message, error) {
	headerConn, err := getHeaderValue("conn", header)
	if err != nil {
		return nil, err
	}
	headerTime, err := getHeaderValue("time", header)
	if err != nil {
		return nil, err
	}
	conn := binary.LittleEndian.Uint32(headerConn)
	time := binary.LittleEndian.Uint64(headerTime)
	return &Message{Conn: conn, Time: time, Data: data}, nil
}

func parseConnection(header, data []byte) (*Connection, error) {
	headerConn, err := getHeaderValue("conn", header)
	if err != nil {
		return nil, err
	}
	headerTopic, err := getHeaderValue("topic", header)
	if err != nil {
		return nil, err
	}
	dataTopic, err := getHeaderValue("topic", data)
	if err != nil {
		return nil, err
	}
	dataType, err := getHeaderValue("type", data)
	if err != nil {
		return nil, err
	}
	dataMD5, err := getHeaderValue("md5sum", data)
	if err != nil {
		return nil, err
	}
	dataMessageDefinition, err := getHeaderValue("message_definition", data)
	if err != nil {
		return nil, err
	}
	...
	dataCallerID, err := getHeaderValue("callerid", data)
	if err == nil {
		id := string(dataCallerID)
		callerID = &id
	}
	dataLatching, err := getHeaderValue("latching", data)
	if err == nil {
		value := dataLatching[0] == 1
		latching = &value
	}

	return &Connection{
		Conn:  binary.LittleEndian.Uint32(headerConn),
		Topic: string(headerTopic),
		Data: ConnectionData{
			Topic:             string(dataTopic),
			Type:              string(dataType),
			MD5Sum:            string(dataMD5),
			MessageDefinition: dataMessageDefinition,
			CallerID:          callerID,
			Latching:          latching,
		},
	}, nil
